...
import {getComp, getGraphType, getK, getMeasure, getThreshold, setMaxThreshold} from "../../../features/compare/compareSlice";
...
const Root = () => {
    ...
    // Load data on mount:
    useEffect(() => {
        fetch(`/api/compare/${graphType}/${measure}/${k}?threshold=${threshold}&names=${'["' + names.join('","') + '"]'}`)
            .then((res) => res.json())
            .then((dataset) => {
                setDataset(dataset);
    ...
    }, [graphType, k, measure, names, threshold]);
